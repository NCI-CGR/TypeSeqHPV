#' ---
#' title: Ion Torrent TypeSeqer HPV Report
#' author: " "
...
TypeSeqer::ion_report_load_packages()
TypeSeqer::ion_report_get_args(is_test=!exists("is_ts"))
parameters_df = TypeSeqer::read_in_parameters_csv(args_parameter_file_path)

#+ A2 summarize hpv type count json, include=FALSE, eval=TRUE, echo=FALSE
bam_header = TypeSeqer::read_in_bam_header(args_bam_header_path)

hpv_types_df = TypeSeqer::create_hpv_types_table(args_hpv_types_path, args_run_manifest_path, bam_header, parameters_df)

#+ A3 Summarize Read Metrics json, include=FALSE, eval=TRUE, echo=FALSE

read_metrics_df = TypeSeqer::create_full_run_read_metrics_df(args_read_metrics_path, 
                                                  hpv_type_counts=hpv_types_df)
scaling_list = TypeSeqer::scaling_of_b2m_human_control(read_metrics_df, 
...
initial_pn_matrix = TypeSeqer::create_inital_pos_neg_matrix(hpv_types_df, scaling_list$factoring_table, scaling_list$filtering_criteria)

control_results = TypeSeqer::calculate_expected_control_results(args_control_defs, initial_pn_matrix)

final_pn_matrix = TypeSeqer::finalize_pn_matrix(initial_pn_matrix, scaling_list$filtering_criteria, control_results$control_results_final)

split_deliverables = TypeSeqer::split_pn_matrix_into_multiple_deliverables(final_pn_matrix, control_results$control_results_final) 

grouped_samples_only_matrix = TypeSeqer::prepare_grouped_samples_only_matrix_outputs(args_custom_groups_path, split_deliverables$samples_only_matrix)

#+ A5 - Prepare Lineage Table Dataframe, include=FALSE, eval=TRUE, echo=FALSE
lineage_df = TypeSeqer::prepare_lineage_df(args_lineage_reference_path, args_lineage_table_path, split_deliverables$samples_only_matrix)

#+ A6 - write all csv files, include=FALSE, eval=TRUE, echo=FALSE
TypeSeqer::write_all_csv_files(final_grouped_samples_only_matrix=grouped_samples_only_matrix, 
                     read_metrics=read_metrics_df,
                     final_pn_matrix = final_pn_matrix, 
                     hpv_types = hpv_types_df, control_matrix = control_results$control_results_final, 
...
get_run_metadata_safe <- possibly(TypeSeqer::get_run_metadata, otherwise=data.frame())
...
sample_summary_safe <- possibly(TypeSeqer::sample_summary, otherwise=data.frame())
...
control_summary_safe <- possibly(TypeSeqer::control_summary, otherwise=data.frame())
...
percent_positive_histogram_safe <- possibly(TypeSeqer::percent_positve_histogram, otherwise=data.frame())
...
coinfection_rate_histogram_safe <- possibly(TypeSeqer::coinfection_rate_histogram, otherwise=data.frame())
...
signal_to_noise_plot_safe <- possibly(TypeSeqer::signal_to_noise_plot, otherwise=data.frame())
...
hpv_status_circle_plot_safe <- possibly(TypeSeqer::hpv_status_circle_plot, otherwise=data.frame())
...
lineage_plot_safe <- possibly(TypeSeqer::lineage_plot, otherwise=data.frame())
