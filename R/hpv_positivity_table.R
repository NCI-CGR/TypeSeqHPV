...
summarize(count = n()) %>%
bind_rows(data_frame(Assay_Plate_Code = "temp",
                     control_result = c("pass", "fail"), count = c(0,0))) %>%
spread(control_result, count, fill = 0) %>%
select(Assay_Plate_Code, `number negative controls passed` = pass,
       `number negative controls failed` = fail)

samples_and_controls_df = split_deliverables$samples_only_matrix %>%
mutate(has_positive = ifelse(Num_Types_Pos == 0,0, 1)) %>%
rename(extraction_plate_id = gDNA_Plate_ID) %>%
group_by(Assay_Plate_Code, extraction_plate_id) %>%
