...
ungroup() %>%
select(-page_num) %>%
select(file_name, bc1_id, total_reads, mapq_greater_than_zero, pass_za, pass_seq_length, pass_mapq) %>%
distinct() %>%
...
select(file_name, bc1_id, mapq_gtz_p, pass_za_p, pass_seq_length_p, pass_mapq_p, qualified_p, total_reads, mapq_greater_than_zero, pass_za, pass_seq_length, pass_mapq, qualified_aligned_reads) %>%
distinct() %>%
glimpse()
}
