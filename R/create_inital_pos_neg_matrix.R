...
create_inital_pos_neg_matrix <- function(hpv_types,
                                         factoring_table,
                                         filtering_criteria) {
...
# set b2m_status
mutate(b2m_status = ifelse(
    (B2M >= factoring_table$B2M_min) | hpv_total_reads >= 1000,
    "pass", "fail")) %>%

# now we set type status to positve and sequentially set to negative if any
#filters are failed
mutate(type_status = "pos") %>%
mutate(type_status = ifelse(hpv_total_reads >= 1000,
                            type_status, "neg"))  %>%
mutate(type_status = ifelse(read_counts >= factored_min_reads_per_type,
                            type_status, "neg"))  %>%
mutate(type_status = ifelse(type_perc >= Min_type_percent_hpv_reads,
                            type_status, "neg")) %>%
group_by(barcode) %>%
mutate(Num_Types_Pos = if_else(type_status == "pos", 1, 0)) %>%
mutate(Num_Types_Pos = sum(Num_Types_Pos)) %>%
mutate(Human_Control = ifelse(b2m_status == "fail",
                              "failed_to_amplify", b2m_status)) %>%
